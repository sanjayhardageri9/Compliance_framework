...
async def test_docker_sandbox_runs_command_with_no_network(context_factory):
    sandbox = DockerSandbox(image="python:3.11-slim", timeout_s=30.0)
    ctx = context_factory()
    req = ToolRequest(
...
async def test_restricted_docker_sandbox_runs_as_non_root(context_factory):
    sandbox = RestrictedDockerSandbox(image="python:3.11-slim", timeout_s=30.0)
    ctx = context_factory()
    req = ToolRequest(
        tool_name="search_tool",
        function_name="query",
...
async def test_docker_sandbox_timeout(context_factory):
    sandbox = DockerSandbox(image="python:3.11-slim", timeout_s=1.0)
    ctx = context_factory()
    req = ToolRequest(
        tool_name="search_tool",
        function_name="query",
